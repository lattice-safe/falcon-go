...
var errSamplerExhausted = errors.New("sampler seed stream exhausted")
var errInvalidFixedSampler = errors.New("invalid fixed nonce or sampler seed")
...
func sign_core(logn uint,
	...
	seed []byte, sig []byte,
	tmp_i16 []int16, tmp_u16 []uint16, tmp_f64 []f64, ct bool) error {

	return sign_core_ext(logn, f, g, F, G, hashed_vk, ctx, id, data, seed, nil, nil, sig, tmp_i16, tmp_u16, tmp_f64, ct)
}

func sign_core_ext(logn uint,
	f []int8, g []int8, F []int8, G []int8,
	hashed_vk []byte, ctx DomainContext, id crypto.Hash, data []byte,
	seed []byte, fixedNonce []byte, fixedSamplerStream []byte, sig []byte,
	tmp_i16 []int16, tmp_u16 []uint16, tmp_f64 []f64, ct bool) error {
	...
	s2 := tmp_i16
	if ct {
		sig = sig[:signatureSizeCT(logn)]
	} else {
		sig = sig[:SignatureSize(logn)]
	}

	var rustPC *shake256prng
	var rustNonce [40]byte
	fixed := fixedNonce != nil
	if fixed {
		if len(fixedNonce) != 40 || len(fixedSamplerStream) < 56 {
			return errInvalidFixedSampler
		}
		copy(rustNonce[:], fixedNonce)
		if err := hash_to_point(logn, rustNonce[:], hashed_vk, ctx, id, data, hm); err != nil {
			return err
		}
	} else if ct {
		rustPC = newSHAKE256prng(seed)
		for i := range rustNonce {
			rustNonce[i] = rustPC.next_u8()
		}
		if err := hash_to_point(logn, rustNonce[:], hashed_vk, ctx, id, data, hm); err != nil {
			return err
		}
	...
	for counter := 0; ; counter++ {
		...
		if fixed {
			nonce = rustNonce
			if fixedSamplerOff+56 > len(fixedSamplerStream) {
				return errSamplerExhausted
			}
			copy(subseed[:], fixedSamplerStream[fixedSamplerOff:fixedSamplerOff+56])
			fixedSamplerOff += 56
			ss = newChaChaSampler(logn, subseed[:])
		} else if ct {
			nonce = rustNonce
			for i := range subseed {
				subseed[i] = rustPC.next_u8()
			}
			ss = newChaChaSampler(logn, subseed[:])
		} else {
			sh.Reset()
		...
		// target size.
		if ct {
			if !trim_i16_encode(logn, s2, maxSigBits[logn], sig[41:]) {
				continue
			}
			sig[0] = byte(0x50 + logn)
			copy(sig[1:41], nonce[:])
			return nil
		}
